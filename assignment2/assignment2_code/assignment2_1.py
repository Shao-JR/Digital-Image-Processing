...
def DFT(img):
    ...
    spectrum = cv2.magnitude(dft_shift[:,:,0],dft_shift[:,:,1])  # calculate spectrum
    spectrum = 20*np.log(spectrum)  # change the scale for better visualization, using 20*np.log()
    phase = cv2.phase(dft_shift[:,:,0],dft_shift[:,:,1])  # calculate phase, output range (-pi, pi)
...
def high_low_pass_filter(dft_shift):
    ...
    low_pass_back = cv2.idft(f_ishift, flags=cv2.DFT_SCALE | cv2.DFT_REAL_OUTPUT)  # transform the new frequency map back to the spatial domain, using cv2.idft().

    # use the high-pass filter to filter out low frequencies
    high_pass_filter = 1 - low_pass_filter  # create a high-pass filter
    ...
    high_pass_back = cv2.idft(f_ishift, flags=cv2.DFT_SCALE | cv2.DFT_REAL_OUTPUT)  # transform the new frequency map back to the spatial domain, using cv2.idft().
