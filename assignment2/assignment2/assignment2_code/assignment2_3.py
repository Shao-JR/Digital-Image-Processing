...
# Introduce to the usage of 'class':
...
#####################################################################################################################
    ...
    def __Encode(self, matrix, tag):
        ...
        # blocks1 = np.lib.stride_tricks.as_strided(matrix, shape=shape, strides=strides)

        res = []  # store the results after DCT and quantization.
...
#####################################################################################################################
    ...
    def __Quantize(self, block, tag):
        if tag == self.__lt:  # for luminance
            # block_quantize = np.round(...) # quantize the block using pre-defined self.__lq, block_quantize has the same shape as block.
            block_quantize = np.round(block / self.__lq).reshape(8, 8)
        elif tag == self.__ct:  # for chrom
            # block_quantize = np.round(...)  # quantize the block using pre-defined self.__cq, block_quantize has the same shape as block.
...
#####################################################################################################################
    def __ZigZag(self, blocks):
        # the motivation of zigzag is to rearrange the pixel positions for better compression (running length coding)
        num_blocks = len(blocks)
        blocks_zigzag = []
        for i in range(num_blocks):
            block = blocks[i]   # block is a reshaped array: 8x8 --> 64
            # block_zigzag = ...  # please scan with zigzag order using pre-defined self.__zigzag_index. block_zigzag has the same shape as block.
...
#####################################################################################################################
    ...
    def __Rle(self, blist):
        # Not required, running length coding.
        # Generally, it tells us how many consecutive zeros before a none-zero value; (number of zeros, value)
        rlist = []
        cnt = 0
        for i in range(len(blist)):
            if blist[i] != 0:
                ...
                cnt = 0
            elif cnt == 15:  # 15 is a pre-defined value in jpeg compression, consecutive zeros > 15 is not allowed 
                rlist.append(cnt)
                rlist.append(int(blist[i]))
                cnt = 0
            else:
    ...
    def Compress(self, filename):
        image = cv2.imread(filename)[:, :, ::-1]  # image read, and BRG ro RGB
        ...
        # zigzag + running length coding
        y_code = self.__Rle(self.__ZigZag(y_blocks))
        u_code = self.__Rle(self.__ZigZag(u_blocks))
        v_code = self.__Rle(self.__ZigZag(v_blocks))

        # Huffman coding can be further adopted in jpeg compression, but not implemented here.
        # you can find a JPEG compression containing Huffman coding with this link:
        # https://github.com/ghallak/jpeg-python

        # write the compressed code into a file bit by bit, not required
    ...
    # write the compressed code into a file, not required
    ...
    # the inverse process of image compression is not required.
...
if __name__ == '__main__':
    # instantiated
    kjpeg = KJPEG()
    # compress the image to '1.gpj'
    kjpeg.Compress("./1.bmp")
    # decompress from the compressed file, line 402 shows the recovered image.
    # decompression process is not required.
