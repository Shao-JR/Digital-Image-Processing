...
def salt_pepper_noise(img):
    ...
    p_salt = 0.1  # probability of salt noise
    p_pepper = 0.1
...
def inverse_filtering(H_shift, img_blur):
    ...
    for v in range(h):
        for u in range(w):
            denominator = H_shift[v, u, 0]**2 + H_shift[v, u, 1]**2
            distance = (v-h_c)**2 + (u-w_c)**2
            if distance > cutoff_H*cutoff_H:
                f_shift[v, u, :] = 0
            else:
                f_shift[v, u, 0] = (G_shift[v, u, 0] * H_shift[v, u, 0] + G_shift[v, u, 1] * H_shift[v, u, 1]) / denominator
                f_shift[v, u, 1] = (G_shift[v, u, 1] * H_shift[v, u, 0] - G_shift[v, u, 0] * H_shift[v, u, 1]) / denominator
    ...
    cv2.imwrite('inverse_filtering_1.png', img_restored)

    plt.subplot(151), plt.imshow(img_blur, cmap='gray')
    plt.title('observed image'), plt.xticks([]), plt.yticks([])
    plt.subplot(152), plt.imshow(img_restored, cmap='gray')
    plt.title('restored_1 image'), plt.xticks([]), plt.yticks([])
    ...
    img_restored = cv2.idft(f_ishift, flags=cv2.DFT_SCALE | cv2.DFT_REAL_OUTPUT)
    img_restored = np.uint8(np.maximum(np.minimum(img_restored, 255), 0))
    cv2.imwrite('inverse_filtering_2.png', img_restored)
    plt.subplot(153), plt.imshow(img_restored, cmap='gray')
    plt.title('restored_2 image'), plt.xticks([]), plt.yticks([])

    threshold = 0.05  # threshold for H_shift, if the magnitude of H_shift is smaller than the threshold, do not change the frequency of observed image.
    for v in range(h):
        for u in range(w):
            denominator = H_shift[v, u, 0]**2 + H_shift[v, u, 1]**2
            H_mag = np.sqrt(denominator)
            if H_mag < threshold:
                f_shift[v, u, :] = G_shift[v, u, :]
            else:
                f_shift[v, u, 0] = (G_shift[v, u, 0] * H_shift[v, u, 0] + G_shift[v, u, 1] * H_shift[v, u, 1]) / denominator
    ...
    img_restored = cv2.idft(f_ishift, flags=cv2.DFT_SCALE | cv2.DFT_REAL_OUTPUT)
    img_restored = np.uint8(np.maximum(np.minimum(img_restored, 255), 0))
    cv2.imwrite('inverse_filtering_3.png', img_restored)
    plt.subplot(154), plt.imshow(img_restored, cmap='gray')
    plt.title('restored_3 image'), plt.xticks([]), plt.yticks([])
...
if __name__=='__main__':
    ...
    #mean_median_fliters(noisy_image)
